...
class ExpMages(MapleJob):
    ...
    def move_horizontal_by(self, displacement, jump_blink=False, attack_blink=True, tolerance_left=2, tolerance_right=2, execute=True):
        ...
        if distance < self.blink_horizontal_distance * 0.8:
            ...
            blinks = int(round((distance + self.distance_between_blinks) / (self.blink_horizontal_distance + self.distance_between_blinks) - 0.2))
        if blinks > 0 and not jump_blink and np.random.random() < 0.3:
            blinks -= 1
        if blinks == 0:
            seq = get_keyPress_seq(arrow_key_code, self.inv_x_t(distance))
        else:
    ...
    def go_to_y(self, current_position, position, need_jump_combo=False, tolerance=2, max_timeout=6):
        ...
        while not is_overlap_y(current_position, position, tolerance) and time.perf_counter() - t0 < max_timeout:
            if current_position.y < position.y:
                ...
                current_platform = self.map.get_platform(current_position)
                on_rope = current_platform is None
                has_rope_down = False
                for rope_down_x in ((current_platform or {}).get("rope_down") or []):
                    if -2 <= rope_down_x - current_position.x <= 2:
                        has_rope_down = True
                for i, level in enumerate(levels):
                    ...
                    if current_position.y < level <= current_position.y + self.blink_vertical_distance:
                        blink_target = level
                if not on_rope and not has_rope_down and (blink_target == current_position.y or blink_target > position.y + tolerance):
                    down_jump()
                elif on_rope or has_rope_down or down_jump_target < position.y - tolerance:
                    down_blink()
                else:
            ...
            elif need_jump_combo or current_position.y - position.y > self.blink_vertical_distance:
            ...
            current_position = get_current_position_of("player", self.map.minimap_region)
        current_platform = self.map.get_platform(current_position)
        if current_platform is None and current_position.y > position.y:
            print(f"climbed down to {current_position.y}, backing up to {position.y}...")
            hold(KEY_UP_ARROW, get_short_delay(3))
            current_position = get_current_position_of("player", self.map.minimap_region)
        elif current_platform is None and current_position.y < position.y:
            print(f"climbed up to {current_position.y}, backing down to {position.y}...")
            hold(KEY_DOWN_ARROW, get_short_delay(3))
            current_position = get_current_position_of("player", self.map.minimap_region)
        elif current_platform is not None:
            if current_position.x in (current_platform.get("rope_down") or []) and current_position.x not in (current_platform.get("rope_up") or []):
                print("climbed down a little, backing up...")
                short_press(KEY_UP_ARROW)
            elif current_position.x not in (current_platform.get("rope_down") or []) and current_position.x in (current_platform.get("rope_up") or []):
                print("climbed up a little, backing down...")
                short_press(KEY_DOWN_ARROW)
        return current_position
    ...
    def go_to(self,
              ...
              teleport_to_position=True,
              tp_count_at_position=0,
              delay_after_rep=0):
        current_position = get_current_position_of("player", self.map.minimap_region)
        if self.map.reset_position is not None and is_overlap(self.map.reset_position, position, tolerance_x, tolerance_y, tolerance_left, tolerance_right) and current_position.y == self.map.bot_level:
            return down_jump(delay_after_rep)
        ...
        if not need_jump_combo:
            for tp_position in self.map.tp_positions:
                if is_overlap(current_position, tp_position.as_position(), 4, 4):
    ...
    def periodically_attack(self, duration, recast_after=0, stop_at=None, max_gap=10, stop_event=None):
        ...
        if self.max_attack_time_gap < 1:
            keyDown(KEY_ATT)
            key_hold_check_time = time.perf_counter()
            while time.perf_counter() < stop_at and not self.check_stop_event_and_simultaneous_events(stop_event):
                if time.perf_counter() - key_hold_check_time > random_unif(3,6):
                    seq = get_keyUp_seq(KEY_ATT, random_unif(0.2, 1.2))
                    ...
                    #     seq = []
                    seq.extend(get_keyDown_seq(KEY_ATT))
                    exec_key_sequence(seq)
                    key_hold_check_time = time.perf_counter()
                short_delay(10)
            keyUp(KEY_ATT)
        ...
        while time.perf_counter() < stop_at and not self.check_stop_event_and_simultaneous_events(stop_event):
            ...
            if np.random.random() < ((time.perf_counter() - t1) / self.max_attack_time_gap) ** 2:
    ...
    def loop(self, rune_cd, dcbot, stop_event=None):
        ...
        while time.perf_counter() - t0 < max_duration and not self.check_stop_event_and_simultaneous_events(stop_event):
            ...
            if self.check_stop_event_and_simultaneous_events(stop_event):
                break
            if self.always_using_booster and self.booster_use_timestamp + 108 < time.perf_counter():
                self.use_booster()
