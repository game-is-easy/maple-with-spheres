...
def back_to_start_position(go_to_fn):
    ...
    return is_overlap(go_to_fn(STARTING_POSITION, need_jump_combo=True), STARTING_POSITION)
...
def setup_placement(go_to_fn):
    ...
    # go_to_fn(SPHERE_POSITION_3, need_jump_combo=True, teleport_to_position=True)
    go_to_fn(TP_POSITION_2, need_jump_combo=True, tolerance_x=1, tolerance_y=2)
    ...
    seq.extend(action_with_prob(short_press, 0.6)(KEY_S, 10, execute=False))
    exec_key_sequence(seq)
...
def loot(go_to_fn):
    ...
    seq = short_press(KEY_ATT, delay_after_rep=10, execute=False)
